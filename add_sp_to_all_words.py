...
"""
Add 'sp' to dictionary entries that don't have it.
Preserves 'sil sil' and 'sp sp' definitions as-is.
"""

import sys
import os

def add_sp_to_dict(input_file, output_file):
    """Add sp to words that don't have it."""
    ...
    for line in lines:
        ...
        if line == "sil sil" or line == "sp sp":
            updated_lines.append(line)
        # If already ends with sp, keep it
        elif len(parts) > 0 and parts[-1] == "sp":
            updated_lines.append(line)
        # Otherwise, add sp
        else:
            updated_lines.append(line + " sp")
            added_count += 1
...
if __name__ == "__main__":
    ...
    for dict_path, dict_name in dict_files:
        ...
        # Backup
        backup_path = dict_path + ".backup"
        os.system(f'cp "{dict_path}" "{backup_path}"')
        print(f"   백업 완료: {backup_path}")
        
        # Add sp
        temp_path = dict_path + ".tmp"
        added = add_sp_to_dict(dict_path, temp_path)
